...
# webcam compartilhada (vision/camera_capture.py).
import difflib
import json
import re
import threading
...
# em abrir_app_local/buscador.py e discord_jarvis/contatos.py pra
...
# main_basic.py) — guardado à parte do ciclo de vida do stream, pra
# que pausar()/retomar() (chamados por GeminiLiveWorker a cada
# chamada, ver gemini/live_client_basic.py) não precisem recebê-lo de
...
# deste projeto (abrir_app_local/buscador.py,
# discord_jarvis/contatos.py, etc.) — copiada aqui de forma
...
# assim main_basic.py nunca trava esperando isso na inicialização do
...
# Chamado uma única vez, em main_basic.py, logo depois que a janela
...
# tomado com a câmera em vision/camera_capture.py). Idempotente: não
...
# termina (ver o cleanup em executar(), gemini/live_client_basic.py).
