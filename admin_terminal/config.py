...
# casa_inteligente/config.py (cada pacote isolado lê o .env sozinho).
...
# admin_terminal/setup.py e admin_terminal/executor.py). Só existe
# depois que 'python -m admin_terminal.setup' for rodado manualmente.
...
# Tarefa Agendada — ver admin_terminal/executor.py e
# admin_terminal/runner_elevado.py.
PASTA_FILA = PASTA_PACOTE / "_fila"
...
# jarvis — ver admin_terminal/executor.py:registrar_log.
ARQUIVO_LOG = PASTA_PACOTE / "comandos_admin.log"

# Lista editável de padrões de comando aprovados para execução
# automática (sem confirmação) — ver admin_terminal/whitelist.py.
...
# INTEGRATION.md, seção "Tela de configurações". Nenhuma variável
