...
On startup `main.py` scans the `.env` (plain Python, no LLM — `jarvis/nucleo/chaves_api.py`) for the API keys basic operation needs: `GEMINI_API_KEY` with the Gemini brain, `OPENAI_API_KEY` with the OpenAI brain, and `GROQ_API_KEY` whenever the tool sub-agent is on (`FERRAMENTAS_SOB_DEMANDA`, default) or the brain is local. If any is missing or blank, `jarvis/ui/janela_chaves_api.py` opens a masked-input window with only the missing ones; what the user saves goes to `.env` and already applies to that same run.
...
- **The API key check in `main.py` must stay before every other `jarvis` import except `jarvis.nucleo.sinalizador`.** `jarvis/nucleo/config.py` and several package `config.py` files copy the key into a module constant at import time, so a key typed in `janela_chaves_api` only applies to the running app because nothing that caches a key was imported yet (`chaves_api.aplicar_no_processo` updates `os.environ` and reloads `jarvis.nucleo.config`, the only such module the check itself touches). Moving `from jarvis.ui.janela_principal import MainWindow` (or any package import) back to the top of `main.py` makes the saved key silently useless until a restart. Which keys count as basic lives in `chaves_api.chaves_necessarias()` — one place; the window never decides it. Covered by `testes/testar_chaves_api.py`, which also checks this import order by AST.
