...
- **Modo JSON com esquema** (`esquema_resposta` da camada de agentes), confirmado ao vivo no `openai/gpt-oss-20b` — isso resolveu a dúvida que estava registrada como "não confirmada" no projeto. Mesmo assim **todo nome que volta é conferido contra o catálogo real**: modo JSON garante sintaxe, nunca conteúdo. Nome inventado é descartado com um aviso no console, nunca repassado.
