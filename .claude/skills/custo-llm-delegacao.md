...
Este documento reflete o estado **atual** do roteador em `delegacao_ia/roteador.py` e
da seção `# DELEGAÇÃO DE TAREFAS` do `instrucao_sistema` em
`gemini/live_client_basic.py`. Releia esses dois arquivos antes de usar esta skill como
...
- Não adicione um segundo caminho de código que chame a OpenAI diretamente (em
  `live_client_basic.py`, num pacote novo, etc.) — `delegar_tarefa(tipo_tarefa=
...
- Se um modelo hardcoded em `delegacao_ia/config.py` (`MODELO_GROQ`/`MODELO_CEREBRAS`/
